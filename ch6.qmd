...
**This nesting is critical to understanding complex systems. The network types most associated with complex systems are scale-free and small-world networks. They are also discussed in Chapter 7. But the network type I often prefer is the hierarchical or nested stochastic block model (HSBM). In the stochastic block model, nodes are organized into clusters, with connections being stronger or more frequent within clusters than between clusters. In the HSBM, clusters are part of larger clusters that form new clusters, and so on.**

**He introduced the concept of near decomposability to describe how these nested hierarchies typically interact. Interactions within each subsystem are generally stronger and more frequent than interactions between subsystems. Of course, the HSBM is a simplification of reality, where levels can be mixed and low-level interference occasionally spirals up to higher levels (see next chapter). Nevertheless, the HSBM is often a useful conceptualization of complex networks, even in psychology.**

![A hierarchical stochastic block model with four levels](media/ch2/network4444.jpeg)
...
The network approach has been applied to many other domains outside of intelligence research and the study of psychopathology. Examples include emotion [@langeEmotionsOverlappingCausal2021], personality [@costantiniStateARtPersonality2015; @cramer2012], interest [@sachisthalIntroducingScienceInterest2019], and organizational behavior [@loweryBalancingActPerformance2021]. One area where it has been developed into a new theory is attitude research.
