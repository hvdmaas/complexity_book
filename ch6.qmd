...
$$
\frac{dX_{i}}{dt} = X_{i} (\sum_{j=1}^{V} \mu_{ij} R_j \overbrace{(1- \frac{X_i}{K_i})}^{\text{Logistic growth}} -  d_i) + \overbrace{ \sum_{j=1}^{W} M_{ij} X_{i} X_{j} / K_{i} }^{\text{Mutualism}}
$$ $$
...
```{r eval = FALSE}
write.table(file='mutualism.txt',data,ro=F,sep='\t')
```
