...
In the network perspective on psychopathology, a mental disorder can be viewed as a system of interacting symptoms (@fig-ch6-img8-old-77). Network theory conceptualizes mental disorders as complex networks of symptoms that interact through feedback loops to create a self-sustaining syndromic constellation. Mental disorders can be understood as alternative stable states of highly interconnected networks of symptoms [@borsboomNetworkTheoryMental2017].
...
The most popular application is to detect which symptoms are central to a disorder [@friedWhatAreGood2016]. However, centrality analysis based on cross-sectional data has its limitations [@bringmannWhatCentralityMeasures2019; @spillerValidityCentralityHypothesis2020]. This is one reason to focus on individual networks using time-series data, often obtained in experience sampling methods (ESMs). Again, these techniques are still under development and not without problems [@dablanderNodeCentralityMeasures2019; @haslbeckRecoveringWithinPersonDynamics2022]. For a recent review of the network approach to psychopathology, see @robinaughNetworkApproachPsychopathology2020.
...
While these problems are not unique to network psychometrics, they are serious problems in practice [see @hamakerModelingAffectDynamics2015; @ryanChallengeGeneratingCausal2022]. The ultimate test of causality requires intervention. Hopefully, the combination of observational and experimental data can provide sufficient information to properly estimate causal relationships in directed acyclic graphs [@dablanderCausalInference2021; @kossakowskiSearchCausalityComparison2021].
