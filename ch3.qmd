...
![A first order (discontinuous) and second order (continuous) phase transition. (from: https://polymerdatabase.com/polymer%20physics/ThemalTransitions.html)](media/ch3/image1.jpg){#fig-ch3-img1 width="5.174305555555556in" height="2.4652777777777777in"}
...
![Transitions in the perception of the Necker cube. The perception of the middle cube is bistable, and sudden transitions occur between the left ('front') and right ('back') percepts.](media/ch3/image2.jpg){#fig-ch3-img2 width="3.6770024059492563in" height="1.2100339020122484in"}
...
[^ch3-1]: Learning a particular conservation task does seem to be rather sudden, but there could easily be two years between learning conservation of number and conservation of volume \[\[CSL STYLE ERROR: reference with no printed form.\]\]. This is inconsistent with the stage theory.
...
![The quadratic potential function](media/ch3/image3.jpg){#fig-ch3-img3 width="2.695424321959755in" height="1.917276902887139in"}
...
![A bifurcation at $a = 0$: the equilibria change in qualitative way.](media/ch3/image4.jpg){#fig-ch3-img4 width="3.985962379702537in" height="2.6466786964129483in"}
...
![The bifurcation diagram of the fold catastrophe](media/ch3/image5.jpg){#fig-ch3-img5 width="4.277777777777778in" height="2.6805555555555554in"}

This bifurcation diagram may not look as spectacular as the logistic map, but its importance cannot be overstated. The fold is everywhere! In a fascinating book, The Seduction of Curves [@mcrobieSeductionCurvesLines2017], McRobie shows that whenever we see an edge, we see a fold. Figure 18 is from the book where he demonstrates how different catastrophes appear in art. I also recommend his YouTube lecture.

![The fold in drawings (from McRobie, 2017).](media/ch3/image6.jpg){#fig-ch3-img6 width="2.540321522309711in" height="3.640939413823272in"}
...
$$ V(X) = {- aX - \frac{1}{2}bX^{2} + \frac{1}{4}X}^{4}. $$ {#eq-ch3-4}
...
![The change in the potential function of the cusp by varying $a$.](media/ch3/image7.jpg){#fig-ch3-img7 width="6.263888888888889in" height="1.3263888888888888in"}
...
![The change in the potential function of the cusp by varying $b$.](media/ch3/image8.jpg){#fig-ch3-img8 width="6.263888888888889in" height="1.3243055555555556in"}
...
![Bifurcation diagrams along the $a$ and $b$ axes. The area in the dotted box is a fold.](media/ch3/image9.jpg){#fig-ch3-img9 width="6.263888888888889in" height="2.1180555555555554in"}
...
![A simple business card can be used to illustrate all the properties of the cusp (see text).](media/ch3/image11.jpg){#fig-ch3-img11 width="2.467130358705162in" height="2.429932195975503in"}
...
<!---![The phase diagram of water, which summarizes the equilibria and transitions in the state of water as a function of temperature and pressure.](media/ch3/image12.jpg){#fig-ch3-img12 width="0.2in" height="0.2in"}--->
...
![The cusp model of attitudes (toward abortion). Because of hysteresis, it is very difficult to persuade highly involved people with new information. But if they change it will be a sudden jump.](media/ch3/image13.jpg){#fig-ch3-img13 width="3.1271905074365702in" height="2.783963254593176in"}
...
Another psychological example of the cusp is multistable perception. [@stewartCatastropheTheoryModeling1983] proposed a model in which the perception of male phase or female figure is used as a behavioral variable, the splitting variable is the amount of detail, and the normal variable is a change in detail related to the male/female distinction. The results are shown in Figure 26.

![From (Stewart & Peregoy, 1983). The fitted bifurcation lines were calculated using Cobb's method, which is explained in section 3.3.3.](media/ch3/image14.jpg){#fig-ch3-img14 width="3.718439413823272in" height="2.6569116360454945in"}
...
![Perturbed pitchfork bifurcation ($a = .1$). For $a = 0$ we would get the pitchfork bifurcation as shown in Figure 21. Thus, a perturbation in a model parameter leads to qualitative change in this bifurcation, and this is the reason why it is not considered structurally stable.](media/ch3/image15.jpg){#fig-ch3-img15 width="3.8476957567804027in" height="2.471142825896763in"}
...
$$
r = \frac{A\ r_{b}}{B}\ and\ q = \frac{K}{A}.$$ {#eq-ch3-8}

Using these two 'constructed' control variables, we can depict the bifurcation lines of the cusp as:

![The bifurcation diagram of the Spruce Budworm model (Copied from Modeling the Spruce Budworm, Jacai deNeveu, 2015).](media/ch3/image16.jpg){#fig-ch3-img16 width="3.2531408573928258in" height="2.981314523184602in"}
...
![Zeeman (1976) dog aggression model with rage and fear as rotated control variables.](media/ch3/image17.jpg){#fig-ch3-img17 width="2.530972222222222in" height="2.1870603674540683in"}
...
![Continuous and discontinuous growth curves my look very similar.](media/ch3/image18.jpg){#fig-ch3-img18 width="3.126003937007874in" height="1.6610793963254593in"}
...
![](media/ch3/image19.jpg){width="3.9362904636920386in" height="2.433779527559055in"}
...
![Bimodality in the expected heights of water when it is poured into a wider glass. This variation of the Piagetian maintenance task is used with children ages 5-8.](media/ch3/image20.jpg){#fig-ch3-img20 width="3.9893274278215225in" height="1.8049267279090113in"}
...
![The pitchfork bifurcation in attitudes. The dotted lines represent the fit of the cusp model to these data. This technique will be discussed in the final section in this chapter.](media/ch3/image21.jpg){#fig-ch3-img21 width="4.123746719160105in" height="2.1345647419072615in"}
...
The basic idea is to make catastrophe theory, a deterministic theory, stochastic by adding a stochastic term, called Wiener noise (with variance $\sigma^{2})$, to Equation $6$[^ch3-4]:

[^ch3-4]: Many different t notations exist for his. Perhaps clearer is $dX(t) = - V^{'}\left( X(t) \right)dt + \sigma dW(t)$, as $dX$ and \$ dW\\ \$ depend on time.

$$
dX = - V^{'}(X)dt + \sigma dW(t).$$ {#eq-ch3-9}
...
$$
f(X) = \frac{1}{{Z\sigma}^{2}}e^{\frac{- V(X)}{\sigma^{2}}},$$ {#eq-ch3-10}

where Z is a normalizing constant[^ch3-5] necessary to ensure that the area under $f(X)$ is 1. This may look complicated but for the quadratic case $V(X) = {\frac{1}{2}X}^{2}$, this results in the standard normal distribution, with $Z = \frac{\sqrt{2\pi}}{\sigma}$.

[^ch3-5]: For consistency with later chapters, I define $Z$ differently from the notation in [@grasmanFittingCuspCatastrophe2009]. It is the inverse of $Z$ in that paper.
...
$$
f(y) = \frac{1}{{Z\sigma}^{2}}e^{\frac{\alpha y + \frac{1}{2}\beta y^{2} - \frac{1}{4}y^{4}}{\sigma^{2}}}.$$ {#eq-ch3-11}
...
![At the back of the cusp (low $b0$), the cusp is approximately linear and cuspfit prefers this simpler model over the cusp model.](media/ch3/image23.jpg){#fig-ch3-img23 width="4.825188101487314in" height="3.0759241032370954in"}

#### Empirical examples

In [@grasmanFittingCuspCatastrophe2009] we present several examples with real data. As another example, we use Stoufer's data, which we used as an example of divergence before (see Figure 33).
...
![Placement of the data of Figure 33 in the bifurcation set.](media/ch3/image24.jpg){#fig-ch3-img24 width="3.8184055118110236in" height="2.8261286089238844in"}
...
![The fit of Bentler conservation data.](media/ch3/image25.jpg){#fig-ch3-img25 width="4.5743646106736655in" height="4.304061679790026in"}
...
![Zeemans's catastrophe machine. One end of the elastic (red dot) is moved by hand through the control plan. The strap point moves according to the cusp catastrophe. Data is gathered by collecting a set of X, Y and Z values. Typically, 50 to 100 data points are sufficient to run Cuspfit.](media/ch3/image26.jpg){#fig-ch3-img26 width="2.497853237095363in" height="3.5110662729658793in"}

#### Evaluation

I like to make a few final remarks. First, Cobb's method is not valid for time series. Data points should be independent. To test for hysteresis in time series other approaches are required. One option is to use hidden Markov models as in [@dutilhPhaseTransitionModel2011].
...
One often speaks of the rise and the fall of catastrophe theory [@rosserRiseFallCatastrophe2007]. The hype following Zeeman's famous Scientific American paper [@zeemanCatastropheTheory1976] led to a strongly worded reply by [@zahlerClaimsAccomplishmentsApplied1977] in Nature.
...
2)  In Zeeman's dog aggression model fear and rage are 'rotated' control variables. How can we translate this to a model with unrotated axes? What would be the definitions of the normal and splitting axis in terms of fear and rage? (\*)

3)  Derive the equation for the bifurcation lines of the cusp ($27a^{2} = 4b^{3})$, by setting the first and second derivatives to zero. (\*\*)

4)  Some insight into the butterfly catastrophe $V(X) = {- aX - bX^{2} - cX}^{3} - dX^{4} + X^{6}$ can be gained by entering the equation in the online graphing calculator GeoGebra. Set a, b, c, d to 0, -5, 0, 5. Then start varying a and c. What is the qualitative difference in the effect of these two parameters? (\*)
