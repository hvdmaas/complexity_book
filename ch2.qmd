...
@eq-ch2-2 is very simple. It is just one equation, a deterministic difference equation specifying how $X_{t + 1}$ depends on $X_{t}$, but the variety of behavior is astonishing. One way to better understand its behavior is to use phase plots. A phase plot is a graphical representation of the relationship between two or more variables that change over time. In one-dimensional systems we plot $X_{t}$ against $X_{t + 1}$ (see @fig-ch2-img7). The code for this figure is:
