...
[^ch2-2]: Why this is not too difficult to understand. If you google 'fixed points of difference equations', you will quickly arrive at stackexchange.com, where several insightful explanations are given.
...
$$
{\lambda_{L} = \lim_{n \rightarrow \infty}}{\sum_{i}^{n}{\ln\left| f^{'}\left( X_{i} \right) \right|}}$$ {#eq-ch2-3}
...
3)  Reproduce the first image from the Henon map wiki page (\*).

4)  Make the bifurcation diagram of the Ricker model (see wiki). What is the advantage of this model over the logistic map?
