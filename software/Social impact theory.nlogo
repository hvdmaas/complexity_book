...
to go
  ...
  tick-advance 100  ;; use `tick-advance`, as we are updating 1000 patches at a time
  update-plots       ;; unlike `tick`, `tick-advance` doesn't update the plots, so we need to do so explicitly
   if (all? patches [ spin = 1 ] or all? patches [ spin  = -1 ]) [ stop ]
...
ask one-of patches
 ...
 let p spin
    ...
    ask other patches [set Is 0
      set Ip 0]
    ask other patches with [spin = p] [
      set Is (supportiveness / ((distancexy px py) ^ 5 ))  *  (1 +  spin *  p) ]
    ask other patches with [spin != p] [
      set Ip (persuasiveness / ((distancexy px py) ^ 5 ))  *  (1 -  spin *  p) ]


  let SIs sum [Is] of patches
  let SIp sum [Ip] of patches
   ;print SIs - SIp

  set spin spin * (SIs - SIp) + h
  set spin spin / abs(spin)

    if spin != p [set sum-of-spins sum-of-spins + 2 * spin]
  recolor
]
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="400000"/>
    <metric>magnetization</metric>
    <enumeratedValueSet variable="max-h">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="probability-of-spin-up">
      <value value="50"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
