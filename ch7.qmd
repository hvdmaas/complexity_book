...
Some important follow-up books to read are @bowlesCooperativeSpeciesHuman2011 on cooperation and altruism, @durlaufSocialDynamics2001 on modeling social economics, the work of @epsteinAgentZeroNeurocognitive2014; @epsteinGenerativeSocialScience2006; @epsteinGrowingArtificialSocieties1996 on building artificial societies and @millerComplexAdaptiveSystems2007 on computation models of social life. There are also some excellent review articles on this area of research [@castellanoStatisticalPhysicsSocial2009; @noorazarRecentAdvancesOpinion2020; @proskurnikovTutorialModelingAnalysis2017]. A less mathematically focused review is provided by @flacheModelsSocialInfluence2017.
...
![BehaviorSpace settings and R-code to visualize the effects of intolerance on segregation.](media/ch7/image1.jpg){#fig-ch7-img1-old-89 width="4.717858705161855in"}
...
![The naming game (adapted from Castellano et al., 2009). The speaker chooses the highlighted word. If the listener does not know the word (failure), she adds it to her inventory. In case of success, both agents delete their inventories, keeping only the spoken word.](media/ch7/image2.jpg){#fig-ch7-img2-old-90 width="3.355262467191601in"}

The game begins with blank lists for all agents. In the first round, the speakers invent a new unique word, assuming that the possibilities are endless. We assume that there is only one object to name and that the social network is fully connected. The R code to simulate this process is:
...
![The order parameter, the total number of unique words, undergoes a phase transition to a state where everyone uses the same word for the object (compare with figure 2 of Castellano et al., 2009).](media/ch7/image3.jpg){#fig-ch7-img3-old-91 width="4.708333333333333in"}
...
![Two runs of Axelrod's model. For 4 features with 4 possible values only one culture remains. For the second case (F = 2, Q = 10) multiple unique cultures, different in all features, emerge.](media/ch7/image4.jpg){#fig-ch7-img4-old-92 width="6.263888888888889in"}
...
![Bottom-up voting in hierarchical systems in the Galam model.](media/ch7/image5.jpg){#fig-ch7-img5-old-93 width="3.0912226596675416in"}
...
![Convergence of opinion in the Friedkin-Johnson model in a connected network of agents.](media/ch7/image7.jpg){#fig-ch7-img7-old-95 width="3.6001038932633422in"}
...
![Four example runs of the Deffuant model with four different boundaries.](media/ch7/image8.jpg){#fig-ch7-img8-old-96 width="6.263888888888889in"}
...
![Clustering in the Hegselmann & Krause model.](media/ch7/image9.jpg){#fig-ch7-img9-old-97 width="4.47871062992126in"}
...
Spontaneous jumps in the dynamical behaviour of this equation are shown in @fig-ch5-img2-old-50 (for $I = 0$, $A_{i} + A^{\min} = 1$ and $s_{O} = 0.4$).
...
![The visualization of persuasion paradox in the original paper. Activism initially spreads quickly, but also increases the attention of conservative agents (see last panel). Over time, opinion polarizes. Red nodes are activists and blue nodes are conservatives. The size of the nodes represents attention.](media/ch7/image11.jpg){#fig-ch7-img11-old-99 width="6.263888888888889in"}
...
The scenario, simulated in @vandermaasPolarizationIndividualsHierarchical2020, consists of a majority of low-involved meat eaters and highly involved vegetarians. The meat eaters refuse to talk to the vegetarians due to bounded confidence, leading to increased polarization.
...
What I really don't like is that the Ising Attitude Model, in which attitudes behave according to the cusp, always leads to radicalization when we get involved. At best, we can jump irregularly between the extreme states (as in @fig-ch5-img2-old-50) as a form of ambivalence. But our society needs people, e.g., judges, who get involved and attend, but at the same time remain neutral. This is impossible in the Ising Attitude Model and thus in the HIOM.
...
1)  Re-run the Schelling simulation (@fig-ch7-img1-old-89) with density of 50 instead of 97. Are the results different? Submit your plot. (\*)
