...
In the HIOM, information and attention are updated based on interactions between agents. Information and attention are two orthogonal axes in the cusp. Information summarizes all variables and influences operating along the normal axis of the cusp. Its neutral value is 0, and negative and positive values are associated with negative and positive opinions. Attention has nonnegative real values. The opinion of agent $i$ is a stochastic differential function (@sec-Cobbs-maximum-likelihood-approach) of information and attention: [^ch7-3]

[^ch7-3]: To incorporate close to linear change in $O$ as a function of $I$, we set $a = A + A^{\min}$, where $A^{\min} = -.5$ and $A ≥ 0$. See the original paper for explanantion.

$$\begin{array}{r}
dO_{i} = \  - \left( O_{i}^{3} - {(A}_{i} + A^{\min})O_{i} - I_{i} \right)dt + s_{O}dW_{i}(t)
\end{array}.$$ {#eq-ch7-3-old-52}

Spontaneous jumps in the dynamical behavior of this equation are shown in @fig-ch4n-img2-old-50 (for $I = 0$, $A_{i} + A^{\min} = 1$ and $s_{O} = 0.4$).
...
$$\begin{array}{r}
\frac{dA_{i}}{dt} = -{decay_{A}{A}}_{i} + {d_{A}u}_{i}.
...
$$\begin{array}{r}
I_{i} = {rI}_{i} + (1 - r)I_{j},
\ where\ r = r_{\min} + \frac{1 - r_{\min}}{1 + e^{- p\left( A_{i} - A_{j} \right)}} \\
...
$$\begin{array}{r}
I_{i} = {decay_{I}(I}_{i} + Ν(m_{I},s_{I}))
