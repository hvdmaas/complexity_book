...
agent-based modeling (Goldstone & Janssen, 2005).
...
Some important follow-up books to read are (Bowles & Gintis, 2011) on
cooperation and altruism, (Durlauf & Young, 2001) on modeling social
economics, the work of (Epstein, 2006, 2014; Epstein & Axtell, 1996)on
building artificial societies and (Miller & Page, 2007) on computation
models of social life. There are also some excellent review articles on
this area of research (Castellano, Fortunato, et al., 2009; Noorazar,
2020; Proskurnikov & Tempo, 2017). A less mathematically focused review
...
Schelling\'s model (Schelling, 1971). The simplified version of this
model assumes a two-dimensional space, a cellular automaton as in
NetLogo, where each location or cell is occupied by one of two types of
agents or is empty. The density parameter determines how many locations
are occupied. Agents stay at their location if a certain percentage of
their 8 neighbors, $B$, are of the same type as the agent. So, if
$B = 0\%$, nobody moves. For values of B close to 100%, everybody moves
all the time. What Schelling showed with his model was that even low
levels of intolerance, near 30%, lead to segregation. Thus, even if
...
keep moving can be seen (Gauvin et al., 2009). Instead of playing around
with the sliders we use the BehavioralSpace option. Figure 89 shows the
settings for this option, the R-code and the results. Note that without
visualizations NetLogo is pretty fast.
...
of similar neighbors. (Gauvin et al., 2009) propose two, probably
better, order parameters, a segregation coefficient, which requires the
identification of clusters, and the density of unwanted locations, which
...
process (Steels, 1995). Steels developed the naming game (Figure 90) to
study the evolution of language (for a review see, (Chen & Lou, 2019).
(Castellano, Fortunato, et al., 2009) provides a brief review.
...
cultures as a simple order parameter. (Castellano et al., 2000) present
detailed analyses of the phase transitions in this model using more
advanced order parameters. As you can imagine, this model can be
extended in many ways, for instance by introducing ordinal instead of
nominal states (Macy et al., 2006).
...
work, except that they are incredibly complex (Newman & Park, 2003).
...
all -1s or all 1s (Castellano, Fortunato, et al., 2009; Redner, 2019).
The probability of ending up in the +1 state is equal to the initial
...
voters on a line ($d = 1$), $NlnN$ for $d = 2$, and $N$ for $d > 2$.
...
such as adding memory and noise to the voters (Castellano, Fortunato, et
al., 2009). It is also possible to consider three groups, left, center,
and right, where left and right do not interact. In this case, depending
on the initial proportion, we end up in a state of full consensus in one
of the states or with a mixture of extremists without centrists (Redner,
2019). Finally, the topology of the social network plays a role. In
general, consensus is reached quickly in scale free networks with broad
...
chooses A and to $v_{i} - a$ if the choice is B. (Martins, 2008)
...
options arise. One option is the Ising model (Galam et al., 1982).
...
Another case is the q-voter model (Castellano, Muñoz, et al., 2009;
Jędrzejewski & Sznajd-Weron, 2019). Here, agents change their opinions
only if all q-voters selected from the neighborhood agree on the other
opinion. When $q = 1$, this reduces to the standard voter model. The
q-voter model generally allows for a higher degree of opinion diversity
compared to the basic voter model. The q-voter model has been
...
as adding an election process (Sznajd-Weron et al., 2021).

#### Social Impact theory

The last discrete model I mention here is the Social Impact Model, which
is based on (Latané, 1981) psychological theory of social impact. Latané
introduced many ideas and concepts from complex systems theory into
social psychology. His psychological theory is firmly grounded in social
psychology and supported by all kinds of evidence (Karau & Williams,
1993).
...
and vice versa. (Lewenstein et al., 1992) present analytical mean-field
solutions for fully connected networks. Without individual fields, the
model ends up with an infinite number of stationary opinion states, one
of which is usually dominant.
...
long periods of time, against all odds (Douglas et al., 2017).
...
influences and identity effects (for a review, see (Holyst et al.,
2001).
...
agents in the network. Thus opinions tend to converge. The
Friedkin-Johnson model (Friedkin & Johnsen, 1990) is an extension that
...
simulated with Grind (using the method=\'euler\' option) by:
...
can also occur in connected networks (Dandekar et al., 2013).
...
al., 2000). The initial opinions of *n* agents are randomly set to
...
the topology of the network does not make much difference (Fortunato,
2004). A drawback of this model is that it converges slowly. A fast but
not entirely accurate code to simulate this model is: [^1]
...
reduces polarization (Zhang & Zhao, 2018). One can also lower the bound
...
(Castellano, Fortunato, et al., 2009) review some other extensions (the
role propaganda, for instance).

Another well-known model is the Hegselmann-Krause (HK) model (Hegselmann
...
Judgment Theory (Sherif & Hovland, 1961). This theory also proposes a
...
investigated in (Jager & Amblard, 2005).
...
(Castellano, Fortunato, et al., 2009) note a striking imbalance between
empirical evidence and theoretical models, in favor of the latter. It is
not that there are no empirical data on the dynamics of opinions. Data
come from studies on voting behavior, multi-country panel surveys,
social media and laboratory studies (for a review see (Peralta et al.,
2022). The problem seems to be that these data do not discriminate
between models. Most of the data fits all opinion models, supporting the
general modeling approach but not specific models. This relates to the
point that current opinion models are difficult to falsify because they
lack specificity and are too flexibility. (Flache et al., 2017) argue
that the field suffers from a lack of systematic comparison of competing
models. The theory construction approach outlined by (Borsboom et al.,
2021) may be helpful here. We need a list of generally agreed phenomena
that all models are supposed to explain.
...
additional psychological variables (Jager, 2017). The Social Impact
model is a good example since it incorporates persuasiveness and
...
Fortunato, et al., 2009). All of these parameters are used to modify the
interactions between agents.

Here I will discuss the model proposed in (van der Maas et al., 2020),
which uses three dynamic variables to describe agents: information,
involvement and opinion. Individual agents are described by the cusp
catastrophe as shown in Figure 25. This is somewhat similar to the work
of (Sobkowicz, 2012), who used the cusp model for the individual agent,
using emotion and information as dynamic control variables. As in our
model, interactions between agents change both opinions and the control
variables. For example, agitation spreads across agents. However, in his
opinion model, Sobkowicz reduces the cusp dynamics to a three-state
system, where opinions are either -1, 0 or 1. We will not adopt this
simplification, as much of the interesting dynamics (hysteresis within
agents) are lost.
...
number of attitudes (e.g., (Chambon et al., 2022; Turner-Zwinkels &
...
review, see (Boccaletti et al., 2014). However, such a model contains an
enormous number of parameters and is hard to study. We take a simpler
approach.
...
social network. In (van der Maas et al., 2020) the results are
...
nodes as in the XY model (Kosterlitz, 1974). Second, we assume
...
the appendix of (van der Maas et al., 2020).
...
disappears. This is the first simulation described in (van der Maas et
al., 2020). The second and third simulation are described in the next
two sections.
...
The scenario, simulated in (van der Maas et al., 2020), consists of a
...
of vegetarianism. For simulation details, see (van der Maas et al.,
2020).
...
also been proposed as a between person opinion model (Barbaro et al.,
2013; Ferri et al., 2022). My dislike in this case is based on limited
evidence. Demonstration with data that increased involvement can lead to
trimodal distribution is needed.
